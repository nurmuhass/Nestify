...
function SellerProfile({ user, onSettings, onMessages }: any) {
  ...
  const sellerInitial = useMemo(() => {
    return sellerDisplayName.charAt(0).toUpperCase();
  }, [sellerDisplayName]);
  ...
  const coverImage = useMemo(() => {
    return firstNonEmpty(user?.cover_image, user?.coverImage);
  }, [user]);

  const sellerCity = useMemo(() => {
    return firstNonEmpty(user?.city, "Nigeria");
  }, [user]);

  const sellerState = useMemo(() => {
    return firstNonEmpty(user?.state, "NG");
  }, [user]);
  ...
  const fetchProperties = useCallback(async () => {
    try {
      ...
      if (approvedResult.status === "success") {
        setApprovedProperties(approvedResult.properties ?? []);
      }

      if (pendingResult.status === "success") {
        setPendingProperties(pendingResult.properties ?? []);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  const tabData = useMemo(() => {
    ...
    return [];
  }, [activeTab, approvedProperties, pendingProperties, estates, staff]);
  ...
  const renderPropertyCard = (item: any) => (
    <TouchableOpacity
      ...
      style={[styles.sellerPropCard, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}
      onPress={() =>
        router.push({
    ...
    >
      <Image
        ...
        style={[styles.sellerPropImg, { backgroundColor: colors.inputBackground }]}
      />
      ...
      <View style={styles.sellerPropOverlay}>
        {isBoosted(item.boosted_until) && (
          <View style={styles.featuredBadge}>
            ...
            <Text style={[styles.featuredBadgeText, { color: colors.background }]}>Featured</Text>
          </View>
        )}
        ...
        >
          <Text style={[styles.sellerStatusText, { color: colors.background }]}>{item.status}</Text>
        </View>
      </View>

      <View style={styles.sellerPropBody}>
        <Text style={[styles.sellerPropName, { color: colors.text }]} numberOfLines={1}>
          {item.propertyName}
        </Text>

        <Text style={[styles.sellerPropPrice, { color: colors.buttonBackground }]}>
          ₦
        ...
        <View style={styles.sellerPropMeta}>
          <Text style={[styles.sellerPropCity, { color: colors.mutedText }]}>{item.city}</Text>

          {item.created_at && (
            <Text style={[styles.sellerPropDate, { color: colors.mutedText }]}>
  ...
  const renderEstateCard = (item: any) => (
    ...
    >
      <Image
        ...
        style={{ width: 90, height: 80, backgroundColor: colors.inputBackground }}
        resizeMode="cover"
      />
      <View style={styles.estateCardBody}>
        <Text style={[styles.estateTitle, { color: colors.text }]} numberOfLines={1}>
          {item.name || item.estate_name || "Untitled Estate"}
        </Text>
        <Text style={[styles.estateSubtitle, { color: colors.mutedText }]} numberOfLines={1}>
          {item.location || item.city || item.state || "No location"}
        </Text>
        <Text style={[styles.estateMeta, { color: colors.buttonBackground }]} numberOfLines={1}>
  ...
  const renderStaffCard = (item: any) => (
    <View key={`staff-${item.id}`} style={styles.staffCard}>
  ...
  const renderTabContent = () => {
    ...
    if (activeTab === "Properties" || activeTab === "Pending") {
      return (
        <View style={styles.propGrid}>{tabData.map(renderPropertyCard)}</View>
      );
    }

    if (activeTab === "Estates") {
      return (
        <View style={styles.tabList}>{estates.map(renderEstateCard)}</View>
      );
    }

    if (activeTab === "Staffs") {
      return <View style={styles.tabList}>{staff.map(renderStaffCard)}</View>;
    }
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sellerHero}>
          ...
          <View style={[styles.heroOverlay, { backgroundColor: colorWithAlpha(colors.background, 0.62) }]} />

          <View style={styles.heroTopBar}>
            <TouchableOpacity style={[styles.heroBtnDark, { backgroundColor: colorWithAlpha(colors.background, 0.72) }]} onPress={onMessages}>
            ...
            <View style={styles.heroTopRight}>
              ...
              <ThemeToggle style={[styles.heroBtnDark, { backgroundColor: colorWithAlpha(colors.background, 0.72) }]} iconColor="#fff" />
              ...
              <TouchableOpacity style={[styles.heroBtnDark, { backgroundColor: colorWithAlpha(colors.background, 0.72) }]} onPress={onSettings}>
          ...
          <View style={styles.heroBottom}>
            <View style={styles.sellerAvatarWrap}>
              {profileImage ? (
                ...
                <View
                  style={[styles.sellerAvatar, styles.sellerAvatarFallback, { backgroundColor: colors.cardBackground }]}
                >
                  <Text style={[styles.sellerAvatarInitial, { color: colors.buttonBackground }]}>
                    {sellerInitial}
                  </Text>
                </View>
              )}
        ...
        <View style={[styles.sellerIdentity, { backgroundColor: colors.background }]}>
          <Text style={[styles.sellerName, { color: colors.text }]}>{sellerDisplayName}</Text>
          <View style={styles.sellerSubRow}>
            <Text style={{ color: colors.text, fontSize: 12, fontWeight: "500" }}>
              {sellerCity}, {sellerState}
            </Text>
            <Text style={[styles.sellerEstablished, { color: colors.mutedText }]}>
              Joined{" "}
              {user.date_established
                ? new Date(user.date_established).getFullYear()
                : "N/A"}
            </Text>
          </View>
          ...
          <View style={[styles.sellerStats, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}>
            <View style={styles.sellerStatItem}>
              <Text style={[styles.sellerStatNum, { color: colors.text }]}>
                {approvedProperties.length + pendingProperties.length}
              </Text>

              <Text style={[styles.sellerStatLbl, { color: colors.mutedText }]}>Properties</Text>
            </View>

            <View style={[styles.sellerStatDivider, { backgroundColor: colors.border }]} />

            <View style={styles.sellerStatItem}>
              <Text style={[styles.sellerStatNum, { color: colors.text }]}>{estates.length}</Text>

              <Text style={[styles.sellerStatLbl, { color: colors.mutedText }]}>Estates</Text>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: "../Profile/UserReviews",
                  params: {
                    company_id: user.id,
                    company_name: sellerDisplayName,
                  },
                })
              }
            >
              ...
              >
                ...
                <MaterialIcons name="star" size={13} color={colors.buttonBackground} />
              </View>

              <Text style={[styles.sellerStatLbl, { color: colors.mutedText }]}>
            ...
            >
              <Text style={[styles.sellerStatNum, { color: colors.text }]}>{saved.length}</Text>

              <Text style={[styles.sellerStatLbl, { color: colors.mutedText }]}>Wishlists</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.sellerCtaRow}>
            <TouchableOpacity
              style={[styles.sellerCtaPrimary, { backgroundColor: colors.buttonBackground }]}
              onPress={() => router.push("../Publish")}
            >
              <Ionicons name="add" size={16} color={colors.background} />

              <Text style={[styles.sellerCtaPrimaryText, { color: colors.background }]}>Add Property</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.sellerCtaSecondary, { backgroundColor: colors.cardBackground, borderColor: colors.buttonBackground }]}
              onPress={() => router.push("../Profile/EditProfile")}
            >
              <Ionicons name="pencil-outline" size={15} color={colors.buttonBackground} />

              <Text style={[styles.sellerCtaSecondaryText, { color: colors.buttonBackground }]}>Edit Profile</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.sellerCtaIcon, { backgroundColor: colors.cardBackground, borderColor: colors.buttonBackground }]}
              onPress={() =>
                router.push({
                  pathname: "../Profile/UserReviews",
                  params: {
            ...
            >
              <MaterialIcons name="rate-review" size={18} color={colors.buttonBackground} />
            </TouchableOpacity>
          </View>
            ...
            >
              <Text
                style={[
                  ...
                  { color: activeTab === tab ? colors.buttonBackground : colors.mutedText },
                ]}
              >
                {tab}
              </Text>

              {activeTab === tab && <View style={[styles.sellerTabUnderline, { backgroundColor: colors.buttonBackground }]} />}
            </TouchableOpacity>
          ))}
        </ScrollView>
